    ...
    while column_index < x_cell_count
      ...
      while row_index < y_cell_count
        ...
        if !@columns[column_index][row_index].north.nil?
          if @columns[column_index][row_index].north.vacant
            neighbor[0] = 1
          end
        end
        if !@columns[column_index][row_index].south.nil?
          if @columns[column_index][row_index].south.vacant 
            neighbor[1] = 1
          end
        end
        if !@columns[column_index][row_index].east.nil?
          if @columns[column_index][row_index].east.vacant 
            neighbor[2] = 1
          end
        end
        if !@columns[column_index][row_index].west.nil?
          if @columns[column_index][row_index].west.vacant 
            neighbor[3] = 1
          end
        end
        

        puts "Cell x: #{column_index}, y: #{row_index}, north: #{neighbor[0]}, south: #{neighbor[1]}, east: #{neighbor[2]}, west: #{neighbor[3]}"
