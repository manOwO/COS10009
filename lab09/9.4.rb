...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      # boundary side
      if @columns[cell_x][cell_y].vacant and (!@columns[cell_x][cell_y].on_path) and (!@columns[cell_x][cell_y].visited)  
        @columns[cell_x][cell_y].on_path = true
        @columns[cell_x][cell_y].visited = true
        if (@columns[cell_x][cell_y].east != nil)  
          return east_path = search(cell_x-1, cell_y)
        elsif (@columns[cell_x][cell_y].west != nil)
          return west_path = search(cell_x+1, cell_y)
        elsif (@columns[cell_x][cell_y].north != nil) 
          return north_path = search(cell_x, cell_y-1)
        elsif (@columns[cell_x][cell_y].south != nil)
          return south_path = search(cell_x, cell_y+1)
        end
        @columns[cell_x][cell_y].on_path = false
        @columns[cell_x][cell_y].visited = false
      end


      # if (@columns[cell_x-1][cell_y].vacant) and (!@columns[cell_x-1][cell_y].on_path) and (!@columns[cell_x-1][cell_y].visited)  
      #   @columns[cell_x-1][cell_y].on_path = true
      #   @columns[cell_x-1][cell_y].visited = true
      #   if (@columns[cell_x][cell_y].east != nil)  
      #     east_path = search(cell_x-1, cell_y)
      #   end
      #   @columns[cell_x-1][cell_y].on_path = false
      #   @columns[cell_x-1][cell_y].visited = false
      # elsif (@columns[cell_x+1][cell_y].vacant) and (!@columns[cell_x+1][cell_y].on_path) and (!@columns[cell_x+1][cell_y].visited) 
      #   @columns[cell_x+1][cell_y].on_path = true
      #   @columns[cell_x+1][cell_y].visited = true
      ...
      #   @columns[cell_x+1][cell_y].on_path = false
      #   @columns[cell_x+1][cell_y].visited = false
      # elsif (@columns[cell_x][cell_y-1].vacant) and (!@columns[cell_x][cell_y-1].on_path) and (!@columns[cell_x][cell_y-1].visited) 
      #   @columns[cell_x][cell_y-1].on_path = true
      #   @columns[cell_x][cell_y-1].visited = true
      #   if (@columns[cell_x][cell_y].north != nil) 
      #     north_path = search(cell_x, cell_y-1)
      #   end
      #   @columns[cell_x][cell_y-1].on_path = false
      #   @columns[cell_x][cell_y-1].visited = false
      # elsif (@columns[cell_x][cell_y+1].vacant) and (!@columns[cell_x][cell_y+1].on_path) and (!@columns[cell_x][cell_y+1].visited) 
      #   @columns[cell_x][cell_y+1].on_path = true
      #   @columns[cell_x][cell_y+1].visited = true
